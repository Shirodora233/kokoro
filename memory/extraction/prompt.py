...
class MemoryExtractionPromptBuilder:
    ...
    def _system_prompt(self) -> str:
        return (
            ...
            "- entity object inside event.entities is only an involvement reference: "
            "use client_id, name, entity_type, identity_summary, aliases, "
            "source_message_ids, and source_quote, but keep properties as an empty "
            "array. If that entity has properties, preferences, states, or other "
            "entity facts, put the full entity exactly once in top-level "
            "entity_candidates and reuse the same client_id in event.entities.\n\n"
            "Event candidates:\n"
            ...
            "- Do not put entity properties inside event.entities. event.entities "
            "only says which entities are involved in the event. Entity facts belong "
            "under the matching top-level entity_candidates item.\n\n"
            ...
            "- Within the same JSON response, reuse one client_id for the same real "
            "entity everywhere. Do not output the same property twice. If an entity "
            "appears in an event and also has properties, event.entities references "
            "the entity with properties: [], while entity_candidates contains the "
            "properties.\n\n"
            ...
            "复诊且地点在静安门诊, event.entities references 用户, 林医生, 静安门诊, "
            "and any entity properties only under entity_candidates.\n"
            "- Input '前几天去吃打抛饭，但是那个吃起来很辣' => output event 吃打抛饭体验 "
            "with description 用户前几天去吃了打抛饭且感觉很辣; event.entities references "
            "用户 and 打抛饭 with properties: []; output top-level entity 打抛饭 with "
            "property 打抛饭吃起来很辣 exactly once.\n"
