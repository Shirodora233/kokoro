...
Run local deterministic tests for candidate matching and normalized prompt
retrieval:
...
link/time-link records. They also cover the normalized search boundary, including
hydrating a description hit back to its parent event without relying on the
recent event/entity pool, plus deterministic ranking rules for search hits.
